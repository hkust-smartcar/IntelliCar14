...
AutoProgram3 *AutoProgram3::m_instance = nullptr;

AutoProgram3::AutoProgram3()
		: m_ccd_filter(2),
		  m_dir_control(&m_car),
...
{
	m_instance = this;
	m_car.UartSendStr("|--- Running Auto 3 ---|\n");
	System::DelayMs(25);
}
...
void AutoProgram3::JoystickPass()
{
	const Timer::TimerInt time = System::Time();
...
void AutoProgram3::SpeedControlPass()
{
	const Timer::TimerInt time = System::Time();
...
void AutoProgram3::DetectEmergencyStop()
{
...
void AutoProgram3::CountDownStage()
{
	m_car.SetBuzzerBeep(true);
	System::DelayMs(100);
	...
	m_car.SetLightSensorListener(0, std::bind(
			&AutoProgram3::OnLightSensorDetectHandler, this, placeholders::_1));
...
void AutoProgram3::OnLightSensorDetectHandler(const uint8_t id)
{
...
int AutoProgram3::FwriteHandler(int, char *ptr, int len)
{
	if (m_instance)
	{
...
void AutoProgram3::HardFaultHandler()
{
	if (m_instance)
	{
